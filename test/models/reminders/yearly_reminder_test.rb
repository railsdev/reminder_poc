...
class YearlyReminderTest < ActiveSupport::TestCase

    def setup
      @reminder = Reminder.create( fq_month: 6, fq_type: 'Yearly', fq_day: 13, fq_time: Time.now )
    end
    ...
    def test_different_of_time_between_scheduled_reminders
      @reminder.scheduled_reminders.order('run_at').to_a.each_cons(2).each{|a, b| assert_equal ((b.run_at - a.run_at)/1.year).round, 1 }
    end
