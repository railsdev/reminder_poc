...
class EveryXDaysReminderTest < ActiveSupport::TestCase

    def setup
      @reminder = Reminder.create( fq_type:    'Every X days', 
                                   fq_time:    Time.now,
                                   start_time: Time.now,
                                   interval:   2 )
    end
    ...
    def test_different_of_time_between_scheduled_reminders
      @reminder.scheduled_reminders.order('run_at').to_a.each_cons(2).each{|a, b| assert_equal ((b.run_at - a.run_at)/1.day).round, @reminder.interval }
    end
