...
class EveryXMonthlyReminderTest < ActiveSupport::TestCase

    def setup
      @reminder = Reminder.create( fq_type:    'Every X months', 
                                   fq_day:     3, 
                                   fq_time:    Time.now,
                                   fq_month:   6,
                                   start_time: Time.now,
    ...
    def test_different_of_time_between_scheduled_reminders
      @reminder.scheduled_reminders.order('run_at').to_a.each_cons(2).each{|a, b| assert_equal ((b.run_at - a.run_at )/1.month).round, @reminder.interval }
    end
